  ...
  try {
    ...
    switch (metric.collect.method) {
      ...
      case 'nvidia-smi':
        ...
        break
      case 'prometheus':
        // Prometheus endpoint requires --metrics flag on llama-server
        // Silently skip if not available — don't log every collection cycle
        try {
          value = await collectPrometheus(metric.collect.field)
        } catch (err) {
          if (err.response?.status === 501 || (err.response && err.response.status === 501)) {
            value = null
          } else {
            throw err
          }
...
async function collectPrometheus(field) {
  ...
  let res
  try {
    res = await axios.default.get('http://127.0.0.1:8080/metrics', { timeout: 5000 })
  } catch (err) {
    // llama-server returns 501 if --metrics flag is not set
