...
app.get('/api/drivers', (req, res) => {
  const drivers = driverRegistry.getAll()
...
wsManager.init(server, async () => {
  const [system, gpu, driversStatus, openclaw] = await Promise.all([
    systemService.getSystemMetrics(),
    systemService.getGpuMetrics(),
    driverRegistry.getAllStatus(),
    openclawService.getStatus(),
  ])
  return { timestamp: new Date().toISOString(), system, gpu, drivers: driversStatus, openclaw }
})

server.listen(PORT, HOST, () => {
  // eslint-disable-next-line no-console
  console.log(`🚀 AI Ops API running on http://${HOST}:${PORT}`)
  // eslint-disable-next-line no-console
...
setInterval(async () => {
  try {
    const [system, gpu, driversMetrics] = await Promise.allSettled([
      systemService.getSystemMetrics(),
      systemService.getGpuMetrics(),
      driverRegistry.getAllMetrics(),
    ])
    const s = system.status === 'fulfilled' ? system.value : {}
    const g = gpu.status === 'fulfilled' ? gpu.value : {}
    const m = driversMetrics.status === 'fulfilled' ? driversMetrics.value : {}

    // Aggregate metrics from all drivers
    let totalTokens = 0
    let totalTps = 0
    for (const driverMetrics of Object.values(m)) {
      if (driverMetrics) {
        totalTokens += driverMetrics['llamacpp:prompt_tokens_total'] || 0
        totalTokens += driverMetrics['llamacpp:tokens_predicted_total'] || 0
        totalTps += driverMetrics['llamacpp:predicted_tokens_seconds'] || 0
      }
    }
