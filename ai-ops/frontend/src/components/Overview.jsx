...
function Overview({ data, loading, error, onRefresh }) {
  ...
  const statusItems = [
    ...
    { label: 'AI', ok: status.llama === 'ok' || status.vllm === 'ok' || status.ollama === 'ok' },
        ...
        <SectionCard title="Токены">
          <div className="grid grid-cols-2 gap-2 text-center">
            <div>
              ...
              <div className="text-sm font-bold text-violet-400">{(tokens.llama || tokens.total)?.toLocaleString('ru-RU') ?? '—'}</div>
            </div>
            <div>
