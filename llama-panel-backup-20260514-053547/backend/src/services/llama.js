...
export async function getSystemdServiceStatus(serviceName) {
  try {
    const { stdout } = await execAsync(`systemctl is-active ${serviceName} 2>/dev/null`)
    const { stdout: statusOut } = await execAsync(
      `systemctl show ${serviceName} --property=ActiveState,SubState,MainPID,LoadState --value 2>/dev/null`
    )
